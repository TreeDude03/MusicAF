...
{
    public sealed partial class FolloweePage : Page
    {
        ...
        private async Task LoadTracksAsync()
        {
            ...
            try
            {
                ...
                var followedTracks = await GetLatestTracksAsync(currentUserEmail);
        ...
        private async Task<List<Track>> GetLatestTracksAsync(string email)
        {
            ...
            try
            {
                ...
                // Fetch users from followed artists
                var followeeRef = _firestoreService.FirestoreDb.Collection("users");
                foreach (var artist in followedArtists)
                {
                    try
                    {
                        var followeeSnapshot = await followeeRef
                            .WhereEqualTo("Name", artist)
                            .GetSnapshotAsync();


                        foreach (var followeeDoc in followeeSnapshot.Documents)
                        {
                            if (followeeDoc.Exists)
                            {
                                var name = followeeDoc.GetValue<string>("Name");
                                try
                                {
                                    var latestTrack = followeeDoc.GetValue<string>("LatestTrackId");
                                    if (!string.IsNullOrEmpty(latestTrack))
                                    {
                                        var trackRef = _firestoreService.FirestoreDb.Collection("tracks").Document(latestTrack);
                                        ...
                                        Console.WriteLine(latestTrack);
                                    }
                                    else
                                    {
                                        Console.WriteLine("Error getting latest track id");
                                    }
